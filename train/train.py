...
class SpeakerDNN(nn.Module):
    def __init__(self, input_dim, num_classes):
        ...
        self.attention = SelfAttention(1024)  # SelfAttention を追加
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)
       # x = self.attention(x)  # SelfAttention を適用
...
BATCH_SIZE = 32
# モデルの初期化
model = SpeakerDNN(INPUT_DIM, NUM_SPEAKERS)
...
class Trainer:
    def __init__(self, base_root, speaker_names):
        ...
        self.init_X_train, self.init_y_train = self.generate_batch_data(
           #  self.train_files, num_segments=20, select_num=20, Data_num=200
...
while True:
    if count == 0:
        ...
        X_train, y_train = trainer.generate_batch_data(
           #  trainer.train_files, num_segments=20, select_num=20, Data_num=200
